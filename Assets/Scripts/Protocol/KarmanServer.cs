...
public class KarmanServer {
    ...
    private void OnConnected(Guid connectionId) {
        ...
        // TODO: broadcast ping messages to all players to calculate latency
        // TODO: send server full message instead of server information packet when to many connections have already been established with the server
        // TODO: disconnect connections if it stays in the Guid.Empty state for X seconds
        // TODO: delete a client after it has been kicked or disconnected after X seconds
        // TODO: add a client secret that is needed before the connection can be taken over
        // TODO: go through all the exceptions and change them to Network or Protocol exceptions based on their position (and handle exceptions everywhere)
        // TODO: add OnConnected/OnReady and OnDisconnected/OnShutdown callbacks to Client and Server
        // TODO: go through all the Debug.Log statements and replace by a layered logging approach
        // TODO: add packet validation, such as a clientId not allowed to be Guid.Empty on an incoming packet in the server
        // TODO: add byte pieces merger/unmerger, that can merge multiple byte pieces into a single byte[] array including the length of the pieces in the merged result. And an unmerger that can take a bytes array, take out (really the out keyword) the next piece, and return the start index for the next piece
    }
